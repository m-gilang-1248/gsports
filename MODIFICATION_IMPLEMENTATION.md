...
- **Action:** Created `lib/core/presentation/widgets/custom_button.dart` with `PrimaryButton` and `SecondaryButton`.
- **Learnings/Surprises:** None.
- **Deviation:** None.
...
## Phase 3: Implement `custom_button.dart`
- [x] Create the file `lib/core/presentation/widgets/custom_button.dart`.
- [x] Implement `PrimaryButton` widget as a `StatelessWidget`, wrapping a `FilledButton` (or `ElevatedButton`).
- [x] Implement `SecondaryButton` widget as a `StatelessWidget`, wrapping an `OutlinedButton`.
- [x] Ensure buttons adhere to the specified height, border radius, and color scheme from `AppColors`.
- [ ] Create/modify unit tests for testing the code added or modified in this phase, if relevant.
- [x] Run the `dart_fix` tool to clean up the code.
- [x] Run the `analyze_files` tool one more time and fix any issues.
- [ ] Run any tests to make sure they all pass.
...
## Phase 4: Implement `custom_text_field.dart`
- [x] Create the file `lib/core/presentation/widgets/custom_text_field.dart`.
- [x] Implement `CustomTextField` widget as a `StatefulWidget`, wrapping a `TextFormField`.
- [x] Add properties for `controller`, `label`, `hint`, `isPassword`, `validator`, `keyboardType`, etc.
- [x] Implement the `isPassword` feature with a visibility toggle (`suffixIcon`).
- [x] Ensure the text field adheres to the specified border styles (enabled, focused, error) using `AppColors`.
- [ ] Create/modify unit tests for testing the code added or modified in this phase, if relevant.
- [x] Run the `dart_fix` tool to clean up the code.
- [x] Run the `analyze_files` tool one more time and fix any issues.
- [ ] Run any tests to make sure they all pass.
